...
```{r}
soal1 <- c(TRUE, "TRUE", 1, 1L)
class(soal1)

soal2 <- c(FALSE, 0L, TRUE, 1)
class(soal2)

soal3 <- c(FALSE, 0L, FALSE, TRUE)
class(soal3)
```
...
```{r}
## Your code here
perumahan <- read.csv("dataset/listings.csv")
...
2. Tinjau output dari `str()` di atas dan jawab pertanyaan berikut:
    ...
    - Apakah variabelnya dalam kelas yang tepat? kota dan kecamatan.
...
```{r}
# Base R
perumahan[1:5, c("kecamatan", "harga", "m2") ]
...
# Please run the code down below
harga_sub_jakbar <- harga_sub_jakbar %>% 
  group_by(kecamatan) %>% # Kolom faktor: yang mau dikelompokkan
  summarise(mean_m2 = mean(harga_m2)) # Kolom numerik: yang dilakukan perhitungan
harga_sub_jakbar
```
